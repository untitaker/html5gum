...
use html5ever::tokenizer::{TokenSink, Token as Html5everToken, TagKind, Tag, TokenSinkResult, Doctype};
use html5ever::tokenizer::states::RawKind;
use crate::{State, Emitter, DefaultEmitter, Token, Error};
...
impl<S: TokenSink> Emitter for Html5everEmitter<S> {
    ...
    fn emit_eof(&mut self) {
        ...
        let _ignored = self.sink.process_token(Html5everToken::EOFToken, BOGUS_LINENO);
        self.sink.end();
    }
    ...
    fn pop_token(&mut self) -> Option<()> {
        ...
        match self.sink.process_token(token_to_html5ever(token), BOGUS_LINENO) {
...
fn token_to_html5ever(token: Token) -> Html5everToken {
    match token {
        Token::StartTag(tag) => {
            Html5everToken::TagToken(Tag {
                kind: TagKind::StartTag,
                name: String::from_utf8_lossy(&*tag.name).into_owned().into(),
                self_closing: tag.self_closing,
                attrs: tag.attributes.into_iter()
                    .map(|(key, value)| {
                        Attribute {
                            name: QualName::new(None, Default::default(), String::from_utf8_lossy(&*key).into_owned().into()),
        ...
        Token::Doctype(doctype) => {
            Html5everToken::DoctypeToken(Doctype {
                name: Some(&*doctype.name).filter(|x| !x.is_empty()).map(|x| String::from_utf8_lossy(x).into_owned().into()),
                public_id: doctype.public_identifier.map(|x| String::from_utf8_lossy(&*x).into_owned().into()),
                system_id: doctype.system_identifier.map(|x| String::from_utf8_lossy(&*x).into_owned().into()),
                force_quirks: doctype.force_quirks,
            })
        }
        Token::Error(err) => {
            Html5everToken::ParseError(err.as_str().into())
        }
    }
}
